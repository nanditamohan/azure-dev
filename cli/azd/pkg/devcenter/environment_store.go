...
func (s *EnvironmentStore) EnvPath(env environment.Persistence) string {
	return fmt.Sprintf("projects/%s/users/me/environments/%s", s.config.Project, env.PersistenceName())
}

// ConfigPath returns the path for the environment configuration
func (s *EnvironmentStore) ConfigPath(env environment.Persistence) string {
	return ""
}
...
func (s *EnvironmentStore) Reload(ctx context.Context, env environment.Persistence) error {
	state, err := env.SnapshotState()
	if err != nil {
		return err
	}
	name := env.PersistenceName()
...
func (s *EnvironmentStore) Save(
	ctx context.Context, env environment.Persistence, options *environment.SaveOptions,
) error {
...
func (s *EnvironmentStore) syncEnvironmentConfig(
	...
	// Don't copy settings already supplied by project or user config into this environment.
	for _, setting := range []struct {
		path, previous, value string
	}{
		{DevCenterNamePath, currentConfig.Name, resolvedConfig.Name},
		{DevCenterProjectPath, currentConfig.Project, resolvedConfig.Project},
		{DevCenterCatalogPath, currentConfig.Catalog, resolvedConfig.Catalog},
		{DevCenterEnvTypePath, currentConfig.EnvironmentType, resolvedConfig.EnvironmentType},
		{DevCenterEnvDefinitionPath, currentConfig.EnvironmentDefinition, resolvedConfig.EnvironmentDefinition},
		{DevCenterUserPath, currentConfig.User, resolvedConfig.User},
	} {
		if setting.previous != "" {
			continue
		}
		if err := cfg.Set(setting.path, setting.value); err != nil {
			return nil, err
		}
	}
