...
func TestEnvironmentStoreReloadRetry(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			env := environment.NewWithValues(remoteEnv.Name, map[string]string{
				"OUTPUT":                      "before",
				"LD_PRELOAD":                  "preserved",
				environment.EnvNameEnvVarName: "not-the-stored-name",
			})
			view := env.Config()
			...
			// A provider view need not expose any variable access to the store.
			persistence := struct{ environment.Persistence }{env}
			if tt.failFirst {
				require.ErrorIs(t, store.Reload(t.Context(), persistence), outputErr)
			...
			require.NoError(t, store.Reload(t.Context(), persistence))
			...
			require.Equal(t, remoteEnv.Name, env.PersistenceName())
