...
func (fs *StorageBlobDataStore) EnvPath(env Persistence) string {
	return fmt.Sprintf("%s/%s", env.PersistenceName(), DotEnvFileName)
}

// ConfigPath returns the path to the config.json file for the given environment
func (fs *StorageBlobDataStore) ConfigPath(env Persistence) string {
	return fmt.Sprintf("%s/%s", env.PersistenceName(), ConfigFileName)
}
...
func (sbd *StorageBlobDataStore) Save(ctx context.Context, env Persistence, options *SaveOptions) error {
	...
	traceSavedName(env.PersistenceName())
	return nil
}

func (sbd *StorageBlobDataStore) Reload(ctx context.Context, env Persistence) error {
	// Reload .env file
	...
	state := PersistedState{Dotenv: envMap, Config: cfg}
