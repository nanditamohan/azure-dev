...
func (fs *LocalFileDataStore) lockPath(env Persistence) string {
	return filepath.Join(fs.azdContext.EnvironmentRoot(env.PersistenceName()), DotEnvFileName+".lock")
}
...
func (fs *LocalFileDataStore) newEnvLock(env Persistence) (*flock.Flock, error) {
...
func (fs *LocalFileDataStore) acquireEnvLock(ctx context.Context, env Persistence) (*flock.Flock, error) {
...
func (fs *LocalFileDataStore) EnvPath(env Persistence) string {
	return filepath.Join(fs.azdContext.EnvironmentRoot(env.PersistenceName()), DotEnvFileName)
}

// ConfigPath returns the path to the config.json file for the given environment
func (fs *LocalFileDataStore) ConfigPath(env Persistence) string {
	return filepath.Join(fs.azdContext.EnvironmentRoot(env.PersistenceName()), ConfigFileName)
}
...
func (fs *LocalFileDataStore) Reload(ctx context.Context, env Persistence) error {
	...
	state := PersistedState{Dotenv: values, Config: cfg}
	if err := env.ReplaceState(state); err != nil {
		return err
	}
	traceLoadedState(env.PersistenceName(), state)
	return nil
}
...
func (fs *LocalFileDataStore) Save(ctx context.Context, env Persistence, options *SaveOptions) error {
	...
	diskValues, err := readDotenv(envPath)
	if err != nil {
		return fmt.Errorf("failed reloading env vars, %w", err)
	}
	var savedState PersistedState
	if err := env.MergeAndSave(saveCtx, diskValues, func(ctx context.Context, state PersistedState) error {
	...
	traceLoadedState(env.PersistenceName(), savedState)
	return nil
}
