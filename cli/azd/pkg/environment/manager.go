...
type Manager interface {
	...
	Save(ctx context.Context, env Persistence) error
	SaveWithOptions(ctx context.Context, env Persistence, options *SaveOptions) error
	Reload(ctx context.Context, env Persistence) error

	// Delete deletes the environment from local storage.
	Delete(ctx context.Context, name string) error

	EnvPath(env Persistence) string
	ConfigPath(env Persistence) string
...
func (m *manager) ConfigPath(env Persistence) string {
	return m.local.ConfigPath(env)
}

// EnvPath returns the path to the environment .env file
func (m *manager) EnvPath(env Persistence) string {
...
func (m *manager) Save(ctx context.Context, env Persistence) error {
	return m.SaveWithOptions(ctx, env, nil)
}

// Save saves the environment to the persistent data store with the specified options
func (m *manager) SaveWithOptions(ctx context.Context, env Persistence, options *SaveOptions) error {
	if options == nil {
		options = &SaveOptions{}
	}
...
func (m *manager) Reload(ctx context.Context, env Persistence) error {
